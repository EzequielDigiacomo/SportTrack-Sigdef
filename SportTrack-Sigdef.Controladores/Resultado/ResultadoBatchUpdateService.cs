...
{
    public class ResultadoBatchUpdateService : IResultadoBatchUpdateService
    {
        ...
        public ResultadoBatchUpdateService(
        ...
        public async Task<IEnumerable<Entidades.Entidades.Resultado>> ApplyBatchUpdateAsync(
            ...
            string? username)
        {
            ...
            foreach (var item in dto)
            {
                ...
                {
                    original.Estado = (EstadoResultadoEnum)Enum.Parse(typeof(EstadoResultadoEnum), item.Estado);

                    if (original.Estado == EstadoResultadoEnum.DNS
                        || original.Estado == EstadoResultadoEnum.DNF
                        || original.Estado == EstadoResultadoEnum.Descalificado)
                    {
                ...
                if (original.Inscripcion?.Participante != null && !string.IsNullOrEmpty(item.ParticipanteNombre))
                {
                    ...
                    if (nameParts.Length > 0)
                    {
                        ...
                        original.Inscripcion.Participante.Apellido = nameParts.Length > 1
                ...
                original.UsuarioActualizacion = username ?? "Sistema";
